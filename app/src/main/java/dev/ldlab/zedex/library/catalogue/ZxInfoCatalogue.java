...
public final class ZxInfoCatalogue implements Catalogue {
    ...
    /**
     * {@code DEFAULT}, and nothing more.
     *
     * <b>{@code TOP} used to be here, sent as {@code sort=score_desc} -
     * measured, in Task 1, to genuinely reorder results.</b> But what it
     * reorders by is Elasticsearch's own relevance score, not a rating -
     * this service keeps no such sort - and that stopped being a private
     * detail the day {@link Item#rating()} started showing a real one on
     * screen. Tried five plausible parameter names for sorting by the
     * rating itself ({@code score.score:desc}, {@code score.score_desc},
     * {@code rating_desc}, {@code votes_desc}, {@code score.votes_desc})
     * against the live service: all five were silently ignored, answering
     * the identical unsorted order - measured against "arkanoid", where
     * relevance ranked a 5-vote, 5/10 entry above a 117-vote, 8.3/10 one.
     * So there is no honest way left to offer something called "Top rated"
     * here: not the sort this service can actually do, which contradicts
     * the very number beside it, and not the rating itself, which this
     * seam has no way to ask the service to order by.
     *
    ...
    private static Item itemFrom(String id, JSONObject source) {
        List<String> images = pictures(source);

        return Item.builder(id)
                .title(text(source, "title"))
                .year(year(source))
                .publisher(publisher(source))
                .kind(text(source, "genreType"))
                .availability(text(source, "availability"))
                .pictureUrl(images.isEmpty() ? null : images.get(0))
                .versions(versions(source))
                // no video link - ZXInfo has nothing to offer here
                .developer(developer(source))
                // ZXDB has no description as such; remarks is the only prose
                // about the game there is - see ZxInfo.meta's own comment,
                // which this mirrors for the same reason that method reads
                // "remarks" rather than a field named for what it becomes.
                .description(text(source, "remarks"))
                .rating(rating(source))
                .images(images)
                // A finer word than kind() where ZXDB has one - the same
                // pairing Meta.subgenre keeps beside Meta.genre, read here
                // for the browsable side of the seam rather than the
                // scraped one.
                .category(text(source, "genreSubType"))
                .build();
    }

    /**
     * Every screen that can be drawn, on whichever host each lives - the
     * catalogue's own picture list, in the record's own order.
     *
     ...
     * thing that decides.
     */
    private static List<String> pictures(JSONObject source) {
    ...
    /**
     * The first author, a person - see {@code ZxInfo.firstAuthor}'s own
     * comment for the same judgement call made for exactly the same reason:
     * ES-DE and ScreenScraper both mean a company by "developer", where ZXDB
     * keeps people and companies apart, and the person who wrote the game is
     * closer to what this word means than the publisher repeated twice.
     */
    private static String developer(JSONObject source) {
        JSONArray authors = source.optJSONArray("authors");
    ...
    /**
     * ZXDB scores out of ten; this reads the same fraction {@code
     * Meta#rating} already uses, so a screen can format either one the same
     * way. See {@code ZxInfo.rating}'s own comment - this is the same field,
     * read for the browsable side of the seam rather than the scraped one.
     */
    private static String rating(JSONObject source) {
        JSONObject score = source.optJSONObject("score");
        if (score == null) return null;

        double outOf10 = score.optDouble("score", -1);
        if (outOf10 < 0) return null;

        return String.format(Locale.US, "%.4f", outOf10 / 10.0);
    }

...
- **ZXInfo blocks an address that arrives without a `User-Agent`**, at the network layer, for hours — DNS still resolving and port 443 actively refused, which from one vantage point is indistinguishable from the host being down. Their spec asks clients to identify themselves and says why. `Http.Real` does, and `ZxInfo` paces itself at 500ms because there is no quota to pace against; neither is optional.
